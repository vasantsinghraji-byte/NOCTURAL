...
describe('Config Infrastructure Validation', () => {
  ...
  describe('ERR-009: Environment loaded before env-sensitive imports', () => {
    it('should call dotenv.config() before requiring env-sensitive local modules and route bootstrap code', () => {
      ...
      const versionRoutesIndex = appSrc.indexOf("require('./routes/v1')");

      expect(dotenvIndex).toBeGreaterThan(-1);
      expect(databaseIndex).toBeGreaterThan(-1);
      expect(rateLimitIndex).toBeGreaterThan(-1);
      expect(appBootstrapIndex).toBeGreaterThan(-1);
      ...
      expect(dotenvIndex).toBeLessThan(appBootstrapIndex);
    });
