...
const PORTALS: Record<LoginPortal, { icon: LucideIcon; fg: string; title: string; tagline: string; tone: string; next: string; demo?: string }> = {
  ...
  pharmacy: { icon: Store, fg: '#1a9960', title: 'Pharmacy partner login', tagline: 'Orders, stock and payouts for your store', tone: 'var(--brand-soft)', next: '/vendor', demo: 'vendor.jaipur@medrush.test / Vendor@123' },
...
export default function PortalLogin({ portal }: { portal: LoginPortal }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.staffLogin(email.trim(), password, portal);
  ...
  return (
    <div className="form">
      <form className="card auth-card" onSubmit={submit}>
        ...
        {cfg.demo && <div className="notice good">Demo: <code>{cfg.demo}</code></div>}
