...
// @access  Private
router.get('/doctor', protect, authorize('doctor', 'nurse'), async (req, res) => {
    try {
...
// @access  Private
router.get('/application-insights/:dutyId', protect, async (req, res) => {
    try {
        ...
        // Optimized: Use aggregation to get all stats in one query
        const [stats, acceptedApp, userApp] = await Promise.all([
            // Get total count
            Application.countDocuments({ duty: req.params.dutyId }),

            // Get accepted application with user details
            Application.findOne({
                duty: req.params.dutyId,
                status: 'ACCEPTED'
            })
            .populate('applicant', 'rating completedDuties')
            .lean(),
...
// @access  Private
router.get('/hospital', protect, authorize('admin'), async (req, res) => {
    try {
...
router.get('/hospital/dashboard', protect, async (req, res) => {
    try {
        ...
        // OPTIMIZED: Get applications with populated data in single query
        ...
        // Calculate key metrics (using in-memory operations on fetched data)
        ...
        // OPTIMIZED: Use Map for better performance than nested object access
