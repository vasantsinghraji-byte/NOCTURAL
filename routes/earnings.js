...
router.post('/', protect, authorize('admin'), async (req, res) => {
    try {
        ...
        const application = await Application.findById(earningData.application);
        ...
        if (String(application.applicant) !== String(earningData.user)) {
            return res.status(400).json({
                success: false,
