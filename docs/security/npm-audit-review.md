...
GeoIP replacement decision: do not add a new provider yet. The current admin geography view is a legacy in-memory rate-limit analytics panel, not a core operational dashboard, and adding a maintained GeoIP provider would require either a database refresh process or an external API key. Keep the production dependency graph clean until the dashboard has a clear owner and acceptance criteria.
