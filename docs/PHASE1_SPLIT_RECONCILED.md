...
> Note (2026-07-13): "Render" above means the canonical `nocturnal-api` service. A second legacy service, `NOCTURAL` (`noctural.onrender.com`), also auto-deploys `main`, was stuck on `e7b220b` after two exit-1 deploys on 2026-07-06, and is being recovered with `REDIS_ENABLED=false` — see the Decision Log and `docs/ops/render-post-deploy-smoke.md`.
