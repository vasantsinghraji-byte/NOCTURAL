...
# Then run: Set-ExecutionPolicy Bypass -Scope Process; .\enable-mongodb-auth-fixed.ps1
...
if (-not $isAdmin) {
    ...
    Write-Host "4. Run: .\enable-mongodb-auth-fixed.ps1" -ForegroundColor Yellow
    Write-Host ""
    pause
    exit 1
}
...
try {
    ...
    if ($service.Status -eq "Running") {
        Write-Host "[OK] MongoDB service restarted successfully" -ForegroundColor Green
    } else {
        Write-Host "[WARNING] MongoDB service status: $($service.Status)" -ForegroundColor Yellow
    }
} catch {
